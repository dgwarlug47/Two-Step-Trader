...
class DA_RNN(nn.Module):
    ...
    def __init__(self, X, y, T,
        ...
        self.d = d
        self.T = T
        # X has dimensions (num_data points, num_features)
        self.X = X
        # y has dimensions (num_data_points)
        self.y = y
        ...
        # Training set
        # self.train_timesteps is the number of data points that will  be analyzed on the training.
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            while (idx < self.train_timesteps):
                ...
                # x has dimensions (batch_size, T - 1, num_features)
                # y_prev has dimension (batch_size, T).
                # y_gt has dimensions (batch_size), and it takes all of the future label. I think.
                y_prev = np.zeros((len(indices), self.T - 1))
                y_gt = self.y[indices + self.T]

                # format x into 3D tensor
                for bs in range(len(indices)):
                    # x has dimensions (batch_size, T, features)
                    # I think that T is basically the seq_length in our model.
                    # But if that is the case that would be weird, because
                    # then we would be looking at the future.
    ...
    def train_forward(self, X, y_prev, y_gt):
        """Forward pass."""
        # X has dimensions (batch_size, T - 1, num_features)
        # y_prev has dimensions(batch_size, T-1)
        # y_prev is exactly what the name says, the previous y or labels.
        ...
        # y_pred has dimension (batch_size, 1)

        import pdb
        pdb.set_trace()

        # y_true has dimensions (batch_size).
